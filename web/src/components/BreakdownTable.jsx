...
export default function BreakdownTable({ rows, dimLabel, onSelect, tiers, features = {}, ticketLabel = {}, showActiveToggle = true }) {
  ...
  const cols = useMemo(() => {
    ...
    if (hasTickets) base.push({ key: 'tickets', label: tShort, fmt: fmtInt });  // 3
    if (hasSpend) {
    ...
    if (hasTickets) base.push({ key: 'ticketRate', label: 'CVR Ticket', fmt: fmtPct }); // 9
    ...
    return base;
  }, [dimLabel, hasSpend, hasImpressions, hasOutbound, hasTickets, hasQuality, tShort]);
