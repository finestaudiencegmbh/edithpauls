...
const TICKET_COL = { key: 'ticket', label: 'VIP', sort: (l) => (l.hasTicket ? 1 : 0) };
const QUALITY_COL = { key: 'quality', label: 'Qualität', sort: (l) => l.quality?.score ?? -1 };

function exportCsv(leads, { hasTickets, hasQuality }) {
  const head = ['Name', 'E-Mail', 'Telefon', 'Lead am', 'Quelle', 'Kampagne', 'Anzeigengruppe', 'Creative', 'Placement'];
  if (hasTickets) head.push('VIP am');
  if (hasQuality) head.push('Quality-Score', 'Tier', 'Einkommen', 'Beschäftigung', 'Immobilien', 'Investiert', 'Beziehungsstand');
  const esc = (v) => `"${String(v ?? '').replace(/"/g, '""')}"`;
  const lines = leads.map((l) => {
    const row = [l.name, l.email, l.phone, l.wonAt, l.sourceType, l.campaign, l.adset, l.creative, l.placement];
    if (hasTickets) row.push(l.ticketAt);
    if (hasQuality) row.push(l.quality?.score ?? '', l.quality?.tier ?? '', l.answers?.income, l.answers?.employment, l.answers?.realEstate, l.answers?.invested, l.answers?.relationship);
...
export default function LeadsTable({ leads, tiers, features = {}, ticketLabel = {} }) {
  const { hasTickets = true, hasQuality = true } = features;
  const COLS = useMemo(() => [
    ...BASE_COLS,
    ...(hasTickets ? [TICKET_COL] : []),
    ...(hasQuality ? [QUALITY_COL] : []),
  ], [hasTickets, hasQuality]);
  ...
  return (
    <div>
      <div className="table-toolbar">
        ...
        <button className="ghost-btn" onClick={() => exportCsv(sorted, { hasTickets, hasQuality })}>
            ...
            {sorted.map((l, i) => {
              ...
              <React.Fragment key={l.email + i}>
                <tr className={`clickable ${isOpen ? 'expanded' : ''}`} onClick={() => setOpen(isOpen ? null : l.email + i)}>
                  ...
                  {hasTickets && <td className="sec" data-label="VIP">{l.hasTicket ? <span className="pill vip">VIP</span> : <span className="muted">–</span>}</td>}
                  {hasQuality && <td data-label="Qualität"><QualityBadge quality={l.quality} tiers={tiers} /></td>}
                </tr>
                {hasQuality && open === l.email + i && l.answers && (
